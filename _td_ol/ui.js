//JAVASCRIPT goi ham lan luot tu tren xuong duoi
(function($hn) {
	'use strict';
	...
	var showArticleContent = function(id) {
		var onCallback = function(article) {
			...
			if (articleContent) {
				...
				articleContent.appendChild(fldiv);
				var currentHref = document.location.href;
				if (currentHref.indexOf("?") > 0)
				{
					var firstPart = currentHref.substring(0, currentHref.indexOf("?"));
					var secondPart = currentHref.substring(currentHref.indexOf("#"));
					currentHref = firstPart + secondPart;
				}
				fldiv.innerHTML = "<br /><fb:like href='" + currentHref + "' show_faces='true' width='718'></fb:like>"
//									+"<fb:comments-count href='" + currentHref + "'/></fb:comments-count> bình luận"
						+ "<fb:comments href='" + currentHref + "' num_posts='10'></fb:comments>";
				FB.XFBML.parse(fldiv);
			}
			else {
...
//				console.log("link.hasClass('story') link.data('hn') === true true");
...
//			}
			...
			var type = link[0].getAttribute("class");
			if (link.hasClass('toggle-submenu')) {
				...
				if (type === 'home') {
					...
					$('.' + CLASS_SHOW_SUBMENU).removeClass(CLASS_SHOW_SUBMENU);
					PubSub.publish('filter-home', type, link.text());
				}	
			}
		}
	};
